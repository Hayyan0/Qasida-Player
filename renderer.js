document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateAllIcons() {
        ...
        document.querySelector('.filter-dropdown-arrow').src = getIconPath('Arrow');
        ...
        updatePlayerFavIcon();
        volumeIcon.src = isMuted ? getIconPath('volume-slash') : getIconPath('volume');
    ...
    function createQasidaItem(qasida, versions, index, forceActiveHeart = false) {
        ...
        const tempAudio = new Audio(`${qasida.directory}/${qasida.file_name}`);
    ...
    function playTrackByData(qasida) {
        currentIndex = currentPlaylist.findIndex(item => item.file_name === qasida.file_name && item.directory === qasida.directory);
        if (currentIndex === -1) {
            currentPlaylist.unshift(qasida);
            currentIndex = 0;
        }
    ...
    function loadTrack(index) {
        ...
        audioPlayer.src = `${qasida.directory}/${qasida.file_name}`;
    ...
    volumeIcon.addEventListener('click', () => {
        volumeIcon.classList.add('icon-swapping');
        volumeIcon.addEventListener('animationend', () => {
            volumeIcon.classList.remove('icon-swapping');
        }, { once: true });
    ...
    window.api.receive('request-state-sync', () => {
        syncPlayerState(true);
    });
